...
// global parameter values #1-#6 (in mm)
double V[7] = {
  0,
  5, //    (SafeHeight)
  1, //   (Depth of Cut)
  0.01, // (XY Scale) modify with -c or -C
  1000.0, //   (Feed) (mm/minute)
  0.0, //    (X offset)
  0.0, //    (Y offset)
  } ;
...
char* fmtR(double r) {
    static char buf[255];
#ifdef NOMACRO
    snprintf(buf, 255, "R%.4f", r*V[3]);
#else
    snprintf(buf, 255, "R[%.4f*#3]", r);
#endif
    return buf ;
}

char* fmtXY(double x, double y) {
    static char buf[255];
#ifdef NOMACRO
    snprintf(buf, 255, "X%.4f Y%.4f", (x*V[3]+V[5]), (y*V[3]+V[6]));
#else
    snprintf(buf, 255, "X[%.4f*#3+#5] Y[%.4f*#3+#6]",  x, y);
#endif
    return buf ;
}
...
#define TTFONT "/System/Library/Fonts/HelveticaLight.ttf"
...
int my_move_to( const FT_Vector* to, void* nodraw )
{
  if (nodraw == NULL) {
#ifdef DXF
    ...
    printf("  0\nVERTEX\n  8\n0\n 10\n%ld.000\n 20\n%ld.000\n 30\n0.0\n",
	   to->x, to->y);
#else
    char *blockdelete = nodraw? "/": "";
    if (debug) printf("(moveto %ld,%ld)\n", to->x, to->y);
#ifdef NOMACRO
    printf("%sG00 Z%.4f\n", blockdelete, V[1]);
    printf("%sG00 %s (moveto)\n", blockdelete, fmtXY(to->x, to->y));
    printf("%sG01 Z%.4f\n", blockdelete, -V[2]);
#else
    printf("%sG00 Z #1\n", blockdelete);
    printf("%sG00 %s (moveto)\n", blockdelete, fmtXY(to->x, to->y));
...
int my_line_to( const FT_Vector* to, void* nodraw )
{
  if (nodraw == NULL) {
#ifdef DXF
    printf("  0\nVERTEX\n  8\n0\n 10\n%ld.000\n 20\n%ld.000\n 30\n0.0\n",
...
int my_conic_to( const FT_Vector* control, const FT_Vector* to, void* nodraw )
{
    int t;
    double x,y;
...
int my_cubic_to(const FT_Vector* control1, const FT_Vector* control2,
                                 const FT_Vector *to, void* nodraw)
{
    int t;
    double x,y;
    FT_Vector point=last_point;
...
static long int render_char(FT_Face face, wchar_t c, long int offset, 
    ...
    error = FT_Outline_Decompose( &outline, &func_interface, nodraw);
...
// calculate the max and min coordinates of the rendered string,
// and return them in the bbox struct
void compute_bbox(char* s, FT_Face face, struct extents* bbox) {
    int l = strlen(s);
    long int offset = 0;
	...
	offset += render_char(face, wc, offset, 0, (void*)1); // does not draw
	extents_add_extents(bbox, &glyph_extents);
	s += r; l -= r;
    }
    printf("(computed bbox: X = %ld to %ld, Y = %ld to %ld)\n",
	   bbox->minx, bbox->maxx, bbox->miny, bbox->maxy);
}

// compute the scale and xy offsets that cause the string to 
// be rendered into the specified bounding box (in machine coordinates).
// place these values into V3,5,6 so that subsequent drawing commands
// will be scaled in this way.
void set_autoscale(char* s, FT_Face face, struct extents* bbox) {
    struct extents sbox ;
    double xscale, yscale;
    compute_bbox(s, face, &sbox);
    xscale = (bbox->maxx - bbox->minx) / (double)(sbox.maxx - sbox.minx) ;
    yscale = (bbox->maxy - bbox->miny) / (double)(sbox.maxy - sbox.miny) ;
    if (xscale > yscale && yscale > 0) {
	V[3] = yscale ;
    } else {
	V[3] = xscale ;
    }
    V[5] = bbox->minx - sbox.minx * V[3];
    V[6] = bbox->miny - sbox.miny * V[3];
}
...
int main(int argc, char **argv) {
    ...
    struct extents bbox = {0, 203, 0, 203} ;
    csteps=100;

    while((i = getopt(argc, argv, "x:y:s:udf:c:C:l:?")) != -1) {
	switch(i) {
	case 's':
	    dsteps=atof(optarg);
	    break;
	...
	case 'u':
        ...
        case 'l':
            ...
            break;
	case 'x':
	    bbox.maxx = atoi(optarg);
	    autoscale = 1 ;
	    break ;
	case 'y':
	    bbox.maxy = atoi(optarg);
	    autoscale = 1 ;
	    break ;
	case '?':
...
#ifdef NOMACRO
    printf("(#1=%.2f    (SafeHeight)\n", V[1]);
    printf("(#2=%.2f   (Depth of Cut)\n", V[2]);
    printf("(#3=%f (XY Scale)\n", V[3]);
    printf("(#4=%.2f   (Feed)\n", V[4]);
    printf("(#5=%.2f    (X offset)\n", V[5]);
    printf("(#6=%.2f    (Y offset)\n", V[6]);
#else
    printf("#1=%.2f    (SafeHeight)\n", V[1]);
    printf("#2=%.2f   (Depth of Cut)\n", V[2]);
    printf("#3=%f (XY Scale)\n", V[3]);
...
#endif
	extents_reset(&glyph_extents);
	offset += render_char(face, wc, offset, linescale, NULL);
